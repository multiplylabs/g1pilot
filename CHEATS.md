...
```
```bash
2.- ros2 launch g1pilot robot_state_launcher.launch.py --ros-args -p use_robot:=True publish_joint_states:=True interface:=eth0
```
```bash
3.- ros2 launch g1pilot navigation_launcher.launch.py --ros-args -p use_robot:=True interface:=eth0 arm_controlled:='both' enable_arm_ui:=True ik_use_waist:=False
```
```bash
